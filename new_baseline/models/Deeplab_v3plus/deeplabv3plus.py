...
from models.Deeplab_v3plus.sync_batchnorm import SynchronizedBatchNorm2d
from torch.nn import BatchNorm2d
from torch.nn import init
from models.Deeplab_v3plus.backbone import build_backbone
from models.Deeplab_v3plus.ASPP import ASPP
...
class deeplabv3plus(nn.Module):
	def __init__(self, cfg):
		...
		self.shortcut_conv = nn.Sequential(
				...
				BatchNorm2d(cfg.MODEL_SHORTCUT_DIM, momentum=cfg.TRAIN_BN_MOM), #Synchronized
				nn.ReLU(inplace=True),		
		)
		self.cat_conv = nn.Sequential(
				nn.Conv2d(cfg.MODEL_ASPP_OUTDIM+cfg.MODEL_SHORTCUT_DIM, cfg.MODEL_ASPP_OUTDIM, 3, 1, padding=1,bias=True),
				BatchNorm2d(cfg.MODEL_ASPP_OUTDIM, momentum=cfg.TRAIN_BN_MOM), #Synchronized
				nn.ReLU(inplace=True),
				nn.Dropout(0.5),
		...
		for m in self.modules():
			...
			elif isinstance(m, BatchNorm2d): #Synchronized
